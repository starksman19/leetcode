...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        max_price = float("-inf")
        min_price = float("inf")
        biggest_diff_this_far = []

        for profit in prices:
            max_price = profit if profit > max_price else max_price
            min_price = profit if profit < min_price else min_price
            biggest_diff_this_far.append(max_price - min_price)

        return max(biggest_diff_this_far)
