...
def get_default_config(run_manager):
    ...
    defaults = {
        ...
        "IS_MODULE_SDK": config.find("option[@name='IS_MODULE_SDK']").attrib.get(
            "value", "true"
        )
        if config.find("option[@name='IS_MODULE_SDK']") is not None
        else "true",
        "ADD_CONTENT_ROOTS": config.find(
            "option[@name='ADD_CONTENT_ROOTS']"
        ).attrib.get("value", "true")
        ...
        "INTERPRETER_OPTIONS": config.find(
            "option[@name='INTERPRETER_OPTIONS']"
        ).attrib.get("value", "")
        if config.find("option[@name='INTERPRETER_OPTIONS']") is not None
        else "",
        "PARENT_ENVS": config.find("option[@name='PARENT_ENVS']").attrib.get(
            "value", "true"
        )
        if config.find("option[@name='PARENT_ENVS']") is not None
        else "true",
        "WORKING_DIRECTORY": config.find(
            "option[@name='WORKING_DIRECTORY']"
        ).attrib.get("value", "$PROJECT_DIR$")
        if config.find("option[@name='WORKING_DIRECTORY']") is not None
        else "$PROJECT_DIR$",
        "SHOW_COMMAND_LINE": config.find(
            "option[@name='SHOW_COMMAND_LINE']"
        ).attrib.get("value", "false")
        ...
        "MODULE_MODE": config.find("option[@name='MODULE_MODE']").attrib.get(
            "value", "false"
        )
        if config.find("option[@name='MODULE_MODE']") is not None
        else "false",
        "REDIRECT_INPUT": config.find("option[@name='REDIRECT_INPUT']").attrib.get(
            "value", "false"
        )
...
def add_run_config_to_workspace(
    ...
    ET.SubElement(
        config, "option", {"name": "ENV_FILES", "value": defaults["ENV_FILES"]}
    )
    ET.SubElement(
        config,
        "option",
    ...
    ET.SubElement(
        config, "option", {"name": "PARENT_ENVS", "value": defaults["PARENT_ENVS"]}
    )
    ...
    ET.SubElement(
        config, "option", {"name": "IS_MODULE_SDK", "value": defaults["IS_MODULE_SDK"]}
    )
    ET.SubElement(
        config,
        "option",
    ...
    ET.SubElement(
        config, "option", {"name": "MODULE_MODE", "value": defaults["MODULE_MODE"]}
    )
    ET.SubElement(
        config,
        "option",
    ...
    ET.SubElement(
        config, "option", {"name": "INPUT_FILE", "value": defaults["INPUT_FILE"]}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_name", default="DefaultRun", help="Nazwa konfiguracji"
    )
    parser.add_argument("--params", default="", help="Parametry uruchomienia")
    parser.add_argument(
        "--interpreter", default=None, help="Ścieżka do interpretera Pythona"
    )
    parser.add_argument(
        "--workspace",
